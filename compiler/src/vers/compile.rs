...
static mut EXPECT: &str = "";
pub(crate) static mut LINE_NUMBER: i32 = 0;
...
pub(crate) unsafe fn compile_vers() {
    ...
    for line in reader.lines() {
        ...
        for mut word in line.unwrap().split_whitespace() {
            ...
            } else if EXPECT == "constname" {
                asm_file.write_fmt(format_args!("{}{}{}{}", "\n", word, CONST, "\n"));
                EXPECT = "consttype";
            } else if EXPECT == "consttype" {
                asm_file.write_fmt(format_args!("{}{}", "    ", check_type(word)));
                EXPECT = "constvalue";
            } else if EXPECT == "constvalue" {
                asm_file.write_fmt(format_args!("{}{}{}", " ", word.replace(";", ""), "\n\n"));
                EXPECT = " "
            }
            // Ignore
            else if word == "{" || word == "}" {
